...
而且查了一下原始的3DGS对于室外场景来说，确实存在难以处理较大的低透明度椭球的问题。
- 解决方案：增大椭球数量上限！顺便换个室内场景试试。

![error](examples/error03_iter5000_psnr17.59.png)


### 长时间迭代但远处模糊
- 现象：整体图形没有问题，但远处的物体（比如纸箱），墙和地板的接缝处模糊。同时点数已经达到了60w，不是未能分裂的问题。
- 原因：原因应该比较明确，是空间中存在较多较大的透明椭球。通过和官方代码的对比，查到了以下几个可能的原因
  - opacity学习率过大：某些高斯在几步内就从“几乎不可见”跳到“很显著”或反过来，导致渲染稳定性下降。同时叠加周期性reset_opacity时，如果学习率偏大，恢复过程会更“急”，更容易出现中远景薄层/雾化样的不稳定贡献。
  - 初始化点云未保留全量：官方代码直接使用了全量的COLMAP点云进行初始化，本地代码考虑到调试时的显存压力仅使用了前30000个
- 解决方案：参考官方代码，试图解决中...

![error](examples/error05_iter30000_psnr24.59.png)
...
- [增] 克隆：对于满足`2D屏幕空间梯度 > 阈值、最大轴尺寸 ≤ 0.01 × scene_extent`的椭球进行复制。新球与原球位置完全一致。目的是增加欠拟合区域的密度，每100步执行一次。
- [增] 分裂：对于满足`2D屏幕空间梯度 > 阈值、最大轴尺寸 > 0.01 × scene_extent`的椭球进行分裂，生成 2 个新球，轴长变为原来的 1/1.6，位置沿最大轴方向随机偏移。目的是将大球精细化，每100步执行一次。
...
然后是Loss：
- [主] loss_L1 = |y_pred - y_gold|，渲染后的图片和真实图片的差异
...
# 吐槽
对于这个`tiny_nerf_data`数据集来说，3DGS真的比Instant-NGP难训太多了：
