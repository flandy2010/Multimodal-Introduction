...
4. SDF network初始化的输出d应该是什么样？既然我们认为相机在球壳上，物体/场景是半径radius=0.8的球，d>0表示在物体外部，那么：
   ...
   - 通过控制初始化可以在network中加上距离先验知识（后面验证这个方案其实也有问题）
```python
def forward(self, x):
...
```
最终实现的效果是：
- 渲染上，在初期eikloss权重很低的时候，随着训练可以看出从一个模糊的状态探索成一个“充气”感觉的挖掘机
...
### 初始化问题
将物体初始化成为一个半径为给定值radius的球体是正确的，但如何实现这件事情也需要注意。
在“归一化”内容里使用了一个简单的方式，在网络中加入先验知识：
```python
# 原始网络输出（初始≈0）+ 距离先验
sdf_raw = ...
sdf = sdf_raw + torch.norm(x, dim=-1, keepdim=True) - self.init_radius
```
这会导致：
- 梯度被模糊，网络学的是sdf_raw，但评估的是 sdf_raw + ||x||，当于给每个采样点加了一个不可训练的"基线偏移"
...
# debug关注点
经过了几轮训练的毒打，总结了一下需要根据哪些统计量来分析模型发生了什么问题：
- Loss：主要看color_loss和eikonal_loss的绝对值和比例，初期eikonal_loss暴涨（到1500都有）也是可以容忍的。
但如果eikonal_loss的权重初始化过大或者不经过warmup突然变大，会出现eikonal_loss极低或者暴跌的情况导致模型突然崩盘
- SDF网络输出：由于经过了合理的归一化，采样路径和采样点覆盖了单位球（同时也不会很超出单位球），同时场景也认为在单位球内，所以SDF物理意义上的合理值应该是[-radius, 2]
  - 如果SDF的最小值都大于0了，通常是模型认为所有点都在模型外部了，这种时候容易崩而且经常拉不回来
  - 如果SDF的最小值保持在-0.5，但最大值激增，通常可以接受。分析来看应该是该处颜色不适合，通过推远距离可以降低alpha，
从而快速降低weight，让颜色变黑（可能和tiny_nerf数据集是黑色背景有关系），但随着训练是可以被拉回来的。
- s：s控制的是
- alpha：alpha或者weight体现的是有多少采样点在真正的贡献颜色