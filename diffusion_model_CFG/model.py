...
from download_dataset import get_image_shape
...
class ConvNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.pe_c = nn.Embedding(11, pe_dim) # 词表大小为11，(0-9)对应数字0-9 + (10)对应emptyset
...
class UNet(nn.Module):
    ...
    def forward(self, x, t, c):
        ...
        n = t.shape[0]
        t = self.pe(t)      # [batch_size, 1, pe_dim]
        c = self.pe_c(c)    # [batch_size, 1, pe_dim]
        condition = t + c   # 将结合了时间t和标签c的condition作为上一个版本的时间t使用

        encoder_outs = []

        for pe_linear, encoder, down in zip(self.pe_linears_en, self.encoders, self.downs):
            # pe = pe_linear(t).reshape(n, -1, 1, 1)
            pe = pe_linear(condition).reshape(n, -1, 1, 1)
            x = encoder(x + pe)
            encoder_outs.append(x)
            x = down(x)

        # pe = self.pe_mid(t).reshape(n, -1, 1, 1)
        pe = self.pe_mid(condition).reshape(n, -1, 1, 1)
        x = self.mid(x + pe)
...
def build_network(config: dict, n_steps, n_classes):
    ...
    network = network_cls(n_steps, n_classes, **config)
